...
//
// ECONOMY ("Bloom Engine"):
//   Growth consumes SUGAR + WATER together; the rate is capped by whichever is
//   scarcer (Liebig's law of the minimum) -> you must balance the two inflows.
//   MINERALS are banked and spent to upgrade the core (faster growth, bigger caps).

export const CONFIG = {
  W: 64,
  H: 40,
  AIR_ROWS: 5,
  start: { sugar: 45, water: 45, mineral: 0 },

  capBase: 60, capStep: 30,        // sugar/water cap = capBase + (coreLevel-1)*capStep
  mineralCap: 160,

  // Growth costs SUGAR + WATER only (the balance pair), scaled by tile difficulty.
  cost: { sugar: 1.2, water: 1.2 },
  costMul: { surface: 0.9, topsoil: 1.0, subsoil: 1.5, rock: 2.6 },

  upkeep: { sugar: 0.03, water: 0.03 }, // per hypha tile, per second

  growthBase: 6, growthPerLevel: 2,     // tiles/sec the frontier advances (scales w/ core level)

  node: {
    sugar:   { rate: 5.0, reserve: 130 },
    water:   { rate: 5.0, reserve: 150 },
    mineral: { rate: 4.0, reserve: 130 },
  },

  upgrade: { base: 25, mult: 1.6 },     // core upgrade cost in minerals: base * mult^(level-1)

  ashBonus: 6,                          // minerals gained when growing into burned (ash) soil
  starveInterval: 0.7,
  coreGrace: 3.0,
  dieBackFrac: 0.04,

  fire: {
    first: 52, interval: 42, accel: 3.5, minInterval: 22,
    warn: 8, sweepDur: 2.6, baseDepth: 2, depthPerFire: 1,
  },
};
...
export function createSim(seed) {
  ...
  for (let i = 0; i < 3; i++) {            // trees with sugar roots
  ...
  for (let i = 0; i < 4; i++) {            // buried logs -> minerals
  ...
  for (let i = 0; i < 4; i++) {            // water pockets
  ...
  const growthRate = () => C.growthBase + (state.coreLevel - 1) * C.growthPerLevel;
  const capFor = (kind) => kind === 'mineral' ? C.mineralCap : (C.capBase + (state.coreLevel - 1) * C.capStep);
  const addRes = (kind, amt) => { state.res[kind] = Math.min(capFor(kind), state.res[kind] + amt); };
  const upgradeCost = () => Math.round(C.upgrade.base * Math.pow(C.upgrade.mult, state.coreLevel - 1));
  ...
  function occupy(t) {
    ...
    if (t.ash) { t.ash = false; addRes('mineral', C.ashBonus); }
  }

  // Frontier advances continuously while SUGAR + WATER are both available.
  // Whichever runs out first becomes the visible bottleneck.
  ...
  function retract(x, y) {
    ...
    t.hypha = false;
    addRes('sugar', C.cost.sugar * t.costMul * 0.3); // small fuel refund
    recomputeNetwork();
  }

  function upgradeCore() {
    const cost = upgradeCost();
    if (state.res.mineral < cost) return false;
    state.res.mineral -= cost; state.coreLevel++;
    return true;
  }
  ...
  function economyStep(dt) {
    ...
    for (const n of state.nodes) {
      ...
      n.reserve -= y; gain[n.kind] += y; addRes(n.kind, y);
    }
    // upkeep drains the growth pair (sugar + water)
    ...
    state.income.mineral = gain.mineral / dt;
    if (deficit) {
  ...
  function gameOver(cause) {
    ...
    state.over = true;
    state.result = { peak: state.peak, time: state.time, seed, cause, size: state.size, coreLevel: state.coreLevel };
  }
  ...
  return { state, tick, growToward, retract, pathTo, pathResourceCost, resourceCost, tileAt, upgradeCore, upgradeCost, CONFIG: C };
}
